  ...
  if (phase === 'planning') {
    return (
      <div className="flex flex-col h-full p-6 space-y-6">
        <div>
          <h2 className="text-2xl font-bold mb-2">Director's Sanctum</h2>
          <p className="text-sm text-[var(--text-secondary)]">
            Paste your narrative and I'll generate a comprehensive Visual Bible
          </p>
        </div>

        <div className="flex-1 flex flex-col gap-4 overflow-hidden">
          <div className="flex-1 flex flex-col">
            <label className="text-xs font medium mb-2">Narrative</label>
            <textarea
              value={narrative}
              onChange={(e) => setNarrative(e.target.value)}
              placeholder="Paste your story, novel, screenplay, or narrative here..."
              className="flex-1 bg-white/50 border border-white/50 rounded p-4 text-sm focus:outline-none focus:border-black/20 resize-none"
            />
          </div>

          <div className="flex flex-col">
            <label className="text-xs font-medium mb-2">Style Notes (Optional)</label>
            <textarea
              value={styleNotes}
              onChange={(e) => setStyleNotes(e.target.value)}
              placeholder="Cyberpunk aesthetic, noir lighting, gritty handheld camera..."
              className="bg-white/50 border border-white/50 rounded p-4 text-sm focus:outline-none focus:border-black/20 resize-none h-24"
            />
          </div>
    ...
    return (
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto">
          {activeTab === 'codex' && (
            <div className="p-6">
              <CodexView visualBible={visualBible} />
            </div>
          )}
