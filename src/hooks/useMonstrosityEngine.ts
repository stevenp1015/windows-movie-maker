...
import { type VisualBible, type SceneNode, type Conversation, type ChatMessage } from '../types';
import * as db from '../services/db';
import { v4 as uuidv4 } from 'uuid';
import { analyzeNarrative, decomposeIntoScenes } from '../services/gemini';
import { ProductionPipeline, type PipelineStatus, type PipelineProgress } from '../services/productionPipeline';
...
export const useMonstrosityEngine = () => {
  ...
  const [systemLogs, setSystemLogs] = useState<Array<{ message: string; type: string; timestamp: number }>>([]);
    ...
    const loadState = async () => {
      try {
        ...
        if (projects && projects.length > 0) {
          const latest = projects.sort((a: any, b: any) => b.lastUpdated - a.lastUpdated)[0];
  ...
  const updateVisualBible = useCallback((updates: Partial<VisualBible>) => {
    setVisualBible((prev) => ({ ...prev, ...updates, lastUpdatedTimestamp: Date.now() }));
  }, []);

  // Generate Visual Bible from narrative
  const generateVisualBible = useCallback(async (narrative: string, styleNotes: string) => {
    try {
  ...
  const generateScenes = useCallback(async (narrative: string) => {
    try {
  ...
  const regenerateScene = useCallback(async (sceneId: string, customPrompt?: string) => {
    ...
    addLog(`Regenerating scene ${scene.index + 1}${customPrompt ? ' with custom prompt' : ''}...`, 'info');
    ...
    // TODO: Trigger actual image generation for this specific scene
    // This will be handled by the production pipeline in the future
    addLog(`Scene ${scene.index + 1} queued for regeneration`, 'info');
  }, [scenes, updateScene, addLog]);

  const forceApproveScene = useCallback((sceneId: string) => {
    const scene = scenes.find(s => s.id === sceneId);
    if (!scene) {
      addLog(`Scene ${sceneId} not found`, 'error');
      return;
  ...
  const injectSceneFeedback = useCallback(async (sceneId: string, feedback: string) => {
    const scene = scenes.find(s => s.id === sceneId);
    if (!scene) {
      addLog(`Scene ${sceneId} not found`, 'error');
      return;
    ...
    addLog(`Processing feedback for scene ${scene.index + 1}: "${feedback.substring(0, 50)}..."`, 'info');

    // Append feedback to current prompt as correction instructions
    const updatedPrompt = `${scene.currentImagePrompt}\n\nUSER FEEDBACK: ${feedback}`;
    
    updateScene(sceneId, { 
      currentImagePrompt: updatedPrompt,
      overallStatus: 'image_generating'
    });

    addLog(`Scene ${scene.index + 1} prompt updated with user feedback`, 'success');
    
    // TODO: Optionally trigger automatic regeneration here
  }, [scenes, updateScene, addLog]);

  const viewSceneImage = useCallback((base64: string) => {
