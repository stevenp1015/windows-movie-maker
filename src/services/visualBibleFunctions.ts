...
export const VISUAL_BIBLE_FUNCTIONS: FunctionDeclaration[] = [
  {
    name: 'addCharacter',
    description: 'Add or update a character in the Visual Bible with detailed appearance and personality information',
    parameters: {
      type: 'object',
      properties: {
        id: { type: 'string', description: 'Unique ID for this character (e.g., "protagonist", "villain")' },
        name: { type: 'string', description: 'Character\'s name' },
        description: { type: 'string', description: 'General character description' },
        keyFeatures: { type: 'array', items: { type: 'string' }, description: 'Array of distinctive features for visual consistency' },
        emotionalArc: { type: 'string', description: 'Character\'s emotional journey' },
        ...
        distinguishingMarks: { type: 'string', description: 'Scars, tattoos, unique marks' }
      },
      required: ['id', 'name', 'description', 'keyFeatures', 'hair', 'eyes', 'build', 'attire']
    }
  },
  {
    name: 'defineSetting',
    description: 'Define a location or environment that appears in the narrative',
    parameters: {
      type: 'object',
      properties: {
        id: { type: 'string', description: 'Unique ID for this setting' },
        name: { type: 'string', description: 'Setting name' },
        locationDescription: { type: 'string', description: 'Detailed description of the location' },
  ...
  },
  {
    ...
    description: 'Set the overall cinematographic style for all generated images',
    parameters: {
      type: 'object',
      properties: {
        lensType: { type: 'string', description: 'Camera lens type (e.g., "35mm prime", "anamorphic")' },
        filmGrain: { type: 'string', description: 'Film grain style (e.g., "subtle 35mm emulation", "digital pristine")' },
        lightingStyle: { type: 'string', description: 'Lighting approach (e.g., "neo-noir high contrast", "soft natural light")' },
        cameraMovement: { type: 'string', description: 'Camera movement style (e.g., "handheld", "smooth dolly")' },
        cameraAngles: { type: 'string', description: 'Preferred camera angles (e.g., "low angles", "dutch tilts")' }
      },
  ...
  },
  {
    ...
    description: 'Define the color palette and visual mood for the entire project',
    parameters: {
      type: 'object',
      properties: {
        mood: { type: 'string', description: 'Overall mood (e.g., "melancholic", "vibrant", "dystopian")' },
        hexCodes: { type: 'array', items: { type: 'string' }, description: 'Array of hex color codes' },
        description: { type: 'string', description: 'Description of how colors should be used' }
      },
  ...
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        themes: { type: 'array', items: { type: 'string' }, description: 'Array of thematic elements' }
      },
      required: ['themes']
    }
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        description: { type: 'string', description: 'What this motif represents' },
        visualExamples: { type: 'array', items: { type: 'string' }, description: 'Visual manifestations of this motif' }
      },
  ...
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        level: { type: 'string', enum: ['Detailed Paragraph', 'Sentence by Sentence', 'Key Beats'], description: 'Granularity level' }
      },
      required: ['level']
    }
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        short: { type: 'number', description: 'Short-term validation stride (e.g., 4)' },
        medium: { type: 'number', description: 'Medium-term validation stride (e.g., 12)' },
        long: { type: 'number', description: 'Long-term validation stride (e.g., 24)' }
      },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        aspectRatio: { type: 'string', description: 'Target aspect ratio (e.g., "16:9", "2.35:1")' },
        maxRetries: { type: 'number', description: 'Max regeneration attempts before user intervention' }
      },
      required: ['name']
    }
  }
  ...
  switch (functionName) {
    ...
    case 'setCinematography':
      updated.cinematography = {
        ...
        lightingStyle: args.lightingStyle || updated.cinematography.lightingStyle,
        cameraMovement: args.cameraMovement || updated.cinematography.cameraMovement,
